...
export class IndexService {
  ...
  async getLocation() {
    ...
    function error(err) {
      switch(err.code) {
        case 1: _.infoUVMessage = 'No se han aceptado los permisos de ubicación.'; break;
        case 2: _.infoUVMessage = 'No se ha podido obtener la ubicación GPS'; break;
        case 3: _.infoUVMessage = 'Se ha agotado el tiempo de espera.'; break;
      }
    }
  }

  async getAllUV(_latitud: number, _longitud: number) {
    const url ='https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/66dae31c35fc699c2030925d35105ca3/';
    const urlData = '?exclude=minutely,hourly,alerts,flags&lang=es&units=auto';

    const valorfinal = url + _latitud + ',' + _longitud + urlData;
    this.http.get( valorfinal )
      .toPromise()
      .then(
        (res) => {
          console.log(res);
